...
from agx_src.replay_buffer_action_sequence import (
    ReplayBufferStorage,
    make_replay_loader,
)
...
class Workspace:
    ...
    def setup(self):
        # create envs
        self.train_env = agx_env.make(
            self.cfg.task_name,
            self.cfg.episode_length,
            self.cfg.frame_stack,
            self.cfg.dataset_root,
            self.cfg.camera_shape,
            self.cfg.stone_height_threshold,
    ...
    def load_agx_demos(self):
        ...
        print(f"Replay buffer size after demos: {len(self.replay_storage)}")
        print(f"Demo buffer size after demos: {len(self.demo_replay_storage)}")
...
@hydra.main(config_path="cfgs", config_name="config_cqn_as_agx")
def main(cfg):
    from train_cqn_as_agx import Workspace as W

    print("Creating workspace...")
    root_dir = Path.cwd()
    workspace = W(cfg)
    print("Workspace created. Loading demos...")
    snapshot = root_dir / "snapshot.pt"
    if snapshot.exists():
        print(f"resuming: {snapshot}")
        workspace.load_snapshot()
    workspace.load_agx_demos()
    print("Demos loaded. Starting training...")
    workspace.train()
